...
This is a Flutter app for Tailg vehicle **official-cloud** control. Local BLE/GATT has been removed. Core Dart code lives in `lib/`: `services/` owns cloud APIs, persistence, control routing, logs, and location; `models/` stores vehicle and telemetry data; `pages/`, `widgets/`, and `theme/` contain UI. Tests live in `test/`. Platform projects are under `android/`, `ios/`, `macos/`, `linux/`, `windows/`; web shell assets are under `web/`. Planning, verification, and build notes belong in `docs/`.
...
Use `flutter_test`. Name test files `*_test.dart` and keep tests close to the behavior they verify, such as `official_cloud_test.dart` or `vehicle_store_test.dart`. Add focused tests for command routing, persistence migration, and UI state changes. Run `dart format`, `flutter analyze`, and `flutter test` before submitting.
...
|------|---------|---------|
| `build.yml` | CI gate (`format` → `analyze` → `test`) for PRs plus signed APK artifact build for branch pushes and manual runs | PR/push to `master` or `develop`, manual |
...
**Quality gates** enforced on every PR and branch push via `build.yml` ci job: `dart format --set-exit-if-changed`, `flutter analyze`, `flutter test`, coverage threshold.

**Build strategy**: PR to `master`/`develop` runs CI automatically; push to `master`/`develop` runs CI and then builds a signed release APK artifact (arm64); manual Build APK workflow can also produce the same artifact; `v*` tags → GitHub Release with APK artifact via `release.yml`. Release signing keys are injected via GitHub Secrets at build time — never committed to the repo.
