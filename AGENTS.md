...
**Quality gates** enforced on every PR and branch push via `build.yml` ci job: `dart format --set-exit-if-changed`, `flutter analyze`, `flutter test`. Coverage reports are **not** currently uploaded (planned in Sprint 2).
