...
![Tests](https://img.shields.io/badge/tests-375_passing-00C896)
...
test/                          57 个测试文件，覆盖云服务、控车路由、持久化与 UI 状态
...
- **暗色**：`AppColorsDark` token 已定义（暗色主色 `#00E0A8`），尚未接入 `ThemeMode`
...
**当前状态**：`format` 0 改动 · `analyze` 0 问题 · **375 / 375 测试通过** · 行覆盖 **81%**（门槛 40%）。
