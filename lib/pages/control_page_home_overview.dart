  ...
  Widget build(BuildContext context) {
    return StreamBuilder<dynamic>(
      ...
      builder: (context, snapshot) {
        ...
        final vehicleName = connectionManager.device?.platformName ??
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(children: [
                  StatusBadge(
                    type: isArmed ? StatusBadgeType.armed : StatusBadgeType.idle,
                  ),
                  const SizedBox(width: 8),
                  StatusBadge(
                    type: isPowerOn ? StatusBadgeType.online : StatusBadgeType.idle,
                    label: isPowerOn ? '已通电' : '未通电',
                  ),
                  const SizedBox(width: 8),
                  if (_isReady) const StatusBadge(type: StatusBadgeType.ble),
                  if (_isReconnecting)
                    const StatusBadge(type: StatusBadgeType.offline, label: '重连中'),
                ]),
              ),
