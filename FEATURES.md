...
- **官方复刻优先**：后续优先接入官方账号登录、官方车辆列表、官方车辆状态和官方云端控车
- **本地 BLE 保底**：云端不可用时，本地车库、BLE 连接和基础控车能力不应受影响
- **官方云桥接来源**：参考 `tailg-ble-web` 的官方短信登录、车辆列表和云端控车接口，迁移到 Flutter 的官方账号模式
...
- **账号与登录体系**：官方验证码登录计划优先复刻；第三方登录、账号注销、换绑手机号、个人资料仍未实现
- **车辆绑定与车库**：官方车辆列表计划优先复刻；扫码绑定、手动绑定、IMEI 绑定、门店绑定、解绑限制仍未实现
- **云端远程能力**：官方云端车辆状态和基础远程控车计划优先复刻；智能服务续费、网络服务充值仍不做
