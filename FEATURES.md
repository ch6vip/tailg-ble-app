...
- **自建云端优先**：后续账号、车辆档案同步、诊断报告、电池快照、操作审计和家庭共享优先走自建后端
- **本地 BLE 保底**：云端不可用时，本地车库、BLE 连接和基础控车能力不应受影响
- **官方云桥接可选**：参考 `tailg-ble-web` 的官方短信登录、车辆列表和云端控车接口，仅作为“官方账号模式 / 实验功能”
- **服务生态分阶段**：先做云同步、多车共享和历史数据，再考虑服务网点、保养记录、消息中心等低耦合能力
...
- **云端方案**：`docs/cloud_architecture_plan.md` 记录自建云端、官方云桥接、多车共享和服务生态规划
