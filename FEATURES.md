...
- **官方账号模式**：已接入官方短信验证码、登录、token 保存/清理、车辆列表和车辆状态模型，待真实账号验证
...
- **账号与登录体系**：官方验证码登录已接入，待真实账号验证；第三方登录、账号注销、换绑手机号、个人资料仍未实现
- **车辆绑定与车库**：官方车辆列表和本地 BLE 关联已接入，待真实账号验证；扫码绑定、手动绑定、IMEI 绑定、门店绑定、解绑限制仍未实现
- **云端远程能力**：官方云端车辆状态和基础远程控车已接入，待真实账号验证；智能服务续费、网络服务充值仍不做
