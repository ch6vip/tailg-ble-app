...
- **官方式首页复刻**：控制页已补未绑定车辆首页、横向功能设置、车辆定位/历史轨迹/NFC/蓝牙续费/充电站等生态卡片，以及 pressed 高亮、缩放、扫光和脉冲执行中反馈
...
- **电池/BMS 详情**：展示电量、电压、温度、信号、预估续航、故障状态；BMS 区按官方 TLV/BMS/C39 字段结构展示 SOC、SOH、电压、容量、电流、温度、循环次数、类型和版本，未确认来源的字段保持“待读取”
...
- **官方账号模式**：已接入官方短信验证码、登录、token 保存/清理、车辆列表和车辆状态模型，已真机验证（2026-06-05，真实官方账号）
- **官方云端控车**：已接入设防、解防、上电、断电、寻车、开坐垫命令映射，支持 BLE / 官方云端 / 自动通道
- **官方车辆详情**：已展示车辆照片、昵称、车架号、官方 IMEI/GPS IMEI、在线、电量、电压、设防、ACC、位置和本地 BLE 车辆关联
- **本地 BLE 保底**：云端不可用时，本地车库、BLE 连接和基础控车能力不应受影响
- **官方云桥接来源**：参考 `tailg-ble-web` 的官方短信登录、车辆列表和云端控车接口迁移到 Flutter
- **自建云端暂停**：`tailg-ble-cloud` 保留为后续工作区，但近期不作为主线
- **服务生态分阶段**：先复刻账号、车辆和控车，再做服务网点、保养记录、消息中心等低风险页面
- **方案文档**：详见 `docs/cloud_architecture_plan.md`
...
- **官方云端入口**：官方账号登录、官方车辆列表、官方车辆详情、官方云端控车通道
- **体验增强**：滑动解锁、感应解锁、基础位置入口、官方车辆与本地 BLE 车辆关联
- **首页 UI 复刻**：未绑定车辆首页、官方白卡控车区、下半生态服务卡、NFC/续费/充电站占位卡和代码动画反馈已按官方 XML 结构复刻

### 官方有但当前未实现或未完整实现

- **账号与登录体系**：官方验证码登录已接入并真机验证（2026-06-05）；第三方登录、账号注销、换绑手机号、个人资料仍未实现
- **车辆绑定与车库**：官方车辆列表和本地 BLE 关联已接入并真机验证（2026-06-05，车辆列表/详情/状态展示通过）；扫码绑定、手动绑定、IMEI 绑定、门店绑定、解绑限制仍未实现
- **云端远程能力**：官方云端车辆状态和基础远程控车已接入并真机验证（2026-06-05，设防/解防/上电等命令真实生效）；智能服务续费、网络服务充值仍不做
...
│   └── official_vehicle.dart    # 官方云端车辆模型
├── ble/
...
│   ├── official_cloud_page.dart # 官方账号、车辆列表、车辆详情和本地关联
│   ├── cloud_token_page.dart    # 旧云 Token 页
│   └── log_page.dart            # 日志查看
├── services/
...
│   └── official_cloud_service.dart # 官方账号、车辆状态和云端控车服务
└── widgets/
