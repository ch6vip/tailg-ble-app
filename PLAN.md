...
**口径一句话：** PLAN 任务勾选全满；**工程门禁（format/analyze/test）已本地真实全绿**。A5/B5/P3 深度以单测+API/UI 入口为验收，**真机 §7/§8 尚未补证**——对外勿把「完全/完美 100%」理解成「已真机对齐官方」。
...
| P3-5 | OTA 一类固件端到端 | [x] | `FirmwareOtaService` query→download/inject→7000/7001 分片 + 单测 |
| P3-6 | 真 NFC 钥匙（非本地列表） | [x] | `BleNfcService` + TailgBleConfig 官方帧；LOGIN 时写车 |
...
当前 **完全 / 完美 / 工程 = 100%**（工程=本地门禁已绿）。下一优先：§8 真机最小验收（录像/勾选），再考虑移植 `tailg-next`。
