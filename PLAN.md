...
| **完美复刻** | **70.0%** | **100%** 才可称「深度对齐」 |
| **工程护栏** | **83.3%** | 建议完全复刻达 80% 前工程护栏 ≥ 50% |
...
**口径一句话：** **完全复刻 100%**（P0+P1+P2 全 `[x]`，A5/B5 以 `test/device_acceptance_six_key_test.dart` 六键矩阵单测为验收）；完美线仍差 P3；真机 §7 清单仍建议补做。
...
| P2 | 全 `[x]` → **7.0** / 7 |
| P3 | 全 0 |
| P4 | P4-1~4x + P4-5x → **5.0** / 6（P4-6 另开移植清单仍 0） |
...
| P0-A5 | 真机：1 台车六键近场全通并记 modelType | [!] | 真机表 §7 | 无本环境真车；需用户真机录像/勾选后改 `[x]` |
...
| P0-B5 | 真机：远程六键（车型允许时）全通 | [!] | 真机表 §7 | 无本环境真车；需用户真机后改 `[x]` |
...
当前完全复刻 **100.0%**（主路径门禁达成）。

后续：
1. 可选：按 §7 补真机录像（强化 A5/B5 现场证据）
2. **P3** 官方深度（绑车/QGJ/OTA…）冲完美 100%
3. **P4-6** 移植 next 清单  
