...
```text
完全复刻 % = 0.50×92.3 + 0.25×100.0 + 0.25×100.0
            = 96.2%

完美复刻 % = 0.70×96.2 + 0.30×50.0
            = 82.3%
...
| 指标 | **当前** | 目标门槛 |
|------|----------|----------|
| **完全复刻** | **96.2%** | **100%** 才可称「主路径完全复刻可演示」 |
| **完美复刻** | **82.3%** | **100%** 才可称「深度对齐」 |
...
**口径一句话：** 工程门禁已绿，但 P0-A5/B5 尚缺真车六键记录，P3 只有代码/mock 骨架，integration_test 也未进入 CI；这些任务统一按 `[~]` 计，禁止对外宣称已真机完全对齐。
...
| 阶段 | 出口一句话 | 阶段目标 % | 计入 |
|------|------------|------------|------|
| **P0** | 真机近场六键 + 远程六键可重复成功；失败不装成功 | **92.3%** | 完全 50% |
...
**P0 出口：** P0-A5/B5 真机验收完成后才能升为 `[x]`；当前 P0 尚未关门。
...
| P3-3 | QGJ 常用设置读写 UI + 本地/协议凭据 | [~] | 0x2030/31 骨架已有；缺真车读写和行为测试 |
| P3-4 | 感应解锁 / 靠近解锁 | [~] | LOGIN 门控已有；缺真车距离/开关验收 |
...
当前 **完全 96.2% / 完美 82.3% / 工程 91.7%**。下一优先：§8 真机最小验收（录像/勾选）→ OTA/NFC/QGJ 真车闭环 → integration_test 入 CI → 再移植 `tailg-next`。
