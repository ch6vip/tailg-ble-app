...
|------|--------|-------|--------|--------------|----------|
| **P0** 通道可证伪 | 13 | 10.0 | **76.9%** | 50% | 38.5 |
| **P1** 爱车/多车 | 6 | 3.5 | **58.3%** | 25% | 14.6 |
| **P2** 数据域 | 7 | 3.0 | **42.9%** | 25% | 10.7 |
| **P3** 官方深度 | 7 | 0.0 | **0.0%** | （完美用） | — |
| **P4** 工程化 | 6 | 1.0 | **16.7%** | （单独） | — |

```text
完全复刻 % = 0.50×76.9 + 0.25×58.3 + 0.25×42.9
            = 38.45 + 14.575 + 10.725
            = 63.8%

完美复刻 % = 0.70×63.8 + 0.30×0.0
            = 44.7%
...
| 指标 | **当前** | 目标门槛 |
|------|----------|----------|
| **完全复刻** | **63.8%** | **100%** 才可称「主路径完全复刻可演示」 |
| **完美复刻** | **44.7%** | **100%** 才可称「深度对齐」 |
| **工程护栏** | **16.7%** | 建议完全复刻达 80% 前工程护栏 ≥ 50% |
...
**口径一句话：** 通道与登出串断已抬升；**真机 + P0 收尾 + P1/P2 数据域** 仍是剩余主路径。
...
| 阶段 | 当前 Σ系数怎么来的（便于手算） |
|------|--------------------------------|
| P0 | A1x A2x A4x B1x B2x B3x B4x C1x C2x + A3~ C3~ → 9×1.0 + 2×0.5 = **10.0** / 13（A5 B5 仍 0） |
| P1 | P1-1~2~3~5~6~ + P1-4x → 5×0.5 + 1.0 = **3.5** / 6 |
| P2 | P2-2~3~4~5~6~7~ → 6×0.5 = **3.0** / 7（P2-1 仍 0） |
| P3 | 全 0 |
| P4 | 仅 P4-5 `[x]` → **1.0** / 6 |
...
1. **P0-A3** — 断蓝牙/离车/杀进程顶栏一致  
2. **P0-C3** — 顶栏四态单一真相源  
3. **P0-A5 / P0-B5** — 真机记录（门禁关键）  

下一阶段主线：P1-4 退出登录串断、P1 空态/busy、P2 去分裂。  

完成一项即 **重算 §0.2**。没有真机结果前，不进入 P3。
