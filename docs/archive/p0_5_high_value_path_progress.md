...
> 上游分析：官方 `HomeActivity` + `ControlFragment` / `UnControlFragment` 跳转显示逻辑  
> **实现演进（2026-07-16）**：爱车 Tab 已改为 Aurora `VehicleControlHomePage`；文中 `ControlPage` / `ControlHomeMode` / `ControlCard` 路径为 **P0.5 当时实现**，代码已移除，仅作历史记录。

本文记录 **A 类高价值优化** 的任务拆分、验收标准与进度。  
不扩本地硬件能力、不接圈子/商城、不做运营弹窗，也不要求真机或实体车辆回归。

相关主线：

- [cloud_only_alignment_progress.md](cloud_only_alignment_progress.md)（P0 已完成）
- [FEATURES.md](../FEATURES.md)
- 当前爱车主页：`lib/pages/vehicle_control_home_page.dart`
...
| 区域 | 路径 |
| --- | --- |
| 爱车主页（Aurora） | `lib/pages/vehicle_control_home_page.dart` |
...
| 服务中心入口 | `lib/pages/service_hub_page.dart` |
| 控车管道 | `lib/services/control_command_executor.dart` 等 |
...
- [x] `dart format` / `flutter analyze` / `flutter test` / coverage 门禁通过
- [x] GitHub Actions CI 与 APK artifact 构建通过
- [x] 真机、实体车辆和 Bluetooth 回归不属于完成定义
...
5. **A5**：全局 `appRouteObserver`；当时挂在 `ControlPage.didPopNext` silent refresh（页已移除；爱车 silent refresh 现由 `main.dart` 生命周期 / Tab 切换与 Aurora 页自行处理）。
...
| 2026-07-12 | 取消真机与本地硬件回归要求，验收统一改为自动化测试和 CI |
| 2026-07-16 | 爱车主页改为 Aurora `VehicleControlHomePage`；删除官方复刻 `ControlPage` 家族；本文路径改为历史记录 |
