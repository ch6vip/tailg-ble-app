...
- 2026-05-29：第二批交互细节优化完成。位置页在无本地 BLE 车时可刷新官方云端坐标；官方账号登录输入增加手机号/验证码校验，未关联官方车辆时突出“关联本地 BLE”；扫描页连接过程中锁定重复点击并显示“连接中”；BLE 控车成功后主动刷新 feb3 状态，失败 SnackBar 可跳转日志；车辆设置中待确认开关/条目点击会解释“暂不开放写入”；电池/BMS 详情用来源标签区分 `BLE feb3 已确认` 和 `官方字段预留`。
- 2026-05-29：稳定性优化第一批完成。官方云控车成功判定与车辆状态刷新解耦，控车接口成功后即返回成功，后续状态刷新失败只写日志；官方云请求日志新增 endpoint、HTTP status、业务 code/msg 和耗时摘要，并写入诊断报告；官方账号服务层补手机号/验证码校验；BLE 连接失败、服务发现失败、Android 133/超时重试前会清理 GATT/特征/挂起命令状态；官方车辆关联本地 BLE 时会清理失效关联并提示重新关联。
...
交互优化待测：
- **官方云位置刷新**：本地车库为空、官方账号已登录并有车辆时，进入车辆位置页点击刷新，应刷新云端经纬度且不提示未绑定本地车辆。
- **官方车辆关联 CTA**：官方账号页未关联车辆时按钮应显示主按钮“关联本地 BLE”；关联后应显示“更换关联”和已关联本地车辆 ID。
- **登录表单校验**：手机号不足 11 位时不能获取验证码；验证码为空或格式不合法时不能登录。
- **扫描防重复连接**：点击设备后该设备显示“连接中”，其他扫描结果显示等待并不可重复连接。
- **控车状态刷新**：BLE 控车成功后应在后续状态区看到 feb3 状态刷新；失败提示应能跳转日志页。
- **云端控车成功判定**：官方云端控车返回成功但后续刷新车辆列表失败时，控车按钮仍应显示成功，日志中记录“指令后刷新状态失败”。
- **BLE 失败恢复**：Android 133 或连接超时后再次连接，应先看到 GATT 清理日志，再进入下一次连接/重连。
- **诊断请求摘要**：诊断报告 Official Cloud 段应包含最后一次官方云请求 endpoint、status、code、耗时和消息摘要。
- **失效关联清理**：删除本地车辆后进入官方账号页，应不再显示旧关联为有效车辆，并可重新关联本地 BLE。
- **设置禁用反馈**：待确认开关和条目点击后应提示“命令待真机验证，暂不开放写入”。
- **BMS 来源标签**：SOC、电压、温度显示 `BLE feb3 已确认`，未接入字段显示 `官方字段预留`。
