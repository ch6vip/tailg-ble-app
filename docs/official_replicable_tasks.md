...
目标：以官方 App 复刻为主线，优先复刻官方账号、车辆列表、云端车辆状态、云端控车、官方页面结构和本地 BLE 兜底能力。所有涉及车端 BLE/ECU 写入的能力必须先完成反编译命令确认和真机验证，再从“待确认”升级为“可实现”。
...
- 2026-05-29：电池/BMS 详情第一批增强。已参考官方 `TlvBatteryInfo`、`BmsBatteryInfoBean`、`BatteryInfoBean`、`QgjBatteryInfoBean` 字段结构，把 BMS 占位升级为字段化只读详情；当前仅把 feb3 已确认的 SOC、电压、温度映射到 BMS 区，SOH、容量、电流、循环次数、类型和版本仍显示“待读取”。
...
- 2026-05-29：修复官方账号登录后仍显示“未绑定车辆”的状态割裂。控制页在本地 BLE 车库为空但官方云已选车辆存在时，会用官方车辆名、云端在线/离线、电量、预估里程和设防/通电状态作为兜底；车辆位置、车辆信息和今日骑行记录也会显示官方车辆名/官方坐标/官方状态兜底。所有兜底均不自动伪造本地 BLE 绑定。
- 2026-05-29：第一批按键细节优化完成。滑动通电/断电增加发送中状态和明确颜色语义；控车不可用时轻点按钮会提示通道原因；快捷编辑按钮扩大触控区但保持小图标视觉；官方验证码按钮增加 60 秒倒计时防重复请求。

## 安全边界

- 可以直接做：页面结构、入口卡片、本地配置、本地记录、只读信息、外部地图跳转、占位说明。
- 可以直接做：用户自有官方账号短信登录、官方车辆列表、官方车辆状态、官方云端基础控车。
- 可以预留入口：NFC、电子围栏、分享用车、骑行记录、消息、设备信息、音效包、服务网点、保养、售后。
- 暂不写入车辆：自动下电、自动锁车、密码解锁、电子龙头锁、边撑感应、坐垫感应、侧翻检测、ECU 骑行高级功能、NFC 加钥匙、真正 OTA。
- 暂不复刻高合规闭环：支付、续费、充电桩交易、保险、商城、金融、官方推送、远程 OTA。
- 禁止绕过：官方登录、官方 token、官方车辆绑定关系。

## 当前优先批：官方账号和云端控车

来源参考：
- `E:\aaatest\tailg-ble-web\src\cloud\api.ts`
- `E:\aaatest\tailg-ble-web\src\cloud\types.ts`
- `E:\aaatest\tailg-ble-web\worker\index.ts`

代码状态：
- 已新增官方云接口服务：验证码、登录、车辆列表、车辆状态、云端控车、token 失效处理。
- 已新增官方账号页面：手机号、验证码、登录态、退出登录。
- 已新增官方车辆列表：车辆名、在线状态、电量、电压、设防、ACC、蓝牙名、蓝牙 MAC。
- 已新增官方车辆详情：车辆照片、昵称、车架号、官方 IMEI/GPS IMEI、命令 IMEI、在线、电量、电压、里程、位置。
- 控制页已增加控车通道：BLE 直连 / 官方云端 / 自动。
- 云端控车已支持：设防、解防、上电、断电、寻车、开坐垫。
- 云端自检已支持：`app/device/cmd/status`，当前保留原始字段展示，字段含义待真车确认。
- 官方车辆和本地 BLE 车辆已支持手动关联；关联后同步本地默认车辆，自动通道避免多车混控。
- 官方 token 和手机号使用本地安全存储，旧偏好数据会迁移并清理。
...
- 云端基础控车有明确成功/失败反馈。
- 官方云失败不影响 BLE 本地控车。
- token、手机号、IMEI 不写入普通日志。
- 待真实账号验证：验证码接口、登录接口、车辆列表接口、云端自检和每条云端控车命令的真实返回。
