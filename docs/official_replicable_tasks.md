...
任务：
- 新增官方云接口服务：验证码、登录、车辆列表、车辆状态、云端控车、token 失效处理。
- 新增官方账号页面：手机号、验证码、登录态、退出登录。
- 新增官方车辆列表：车辆名、在线状态、电量、电压、设防、ACC、蓝牙名、蓝牙 MAC。
- 控制页增加控车通道：BLE 直连 / 官方云端 / 自动。
- 云端控车支持：设防、解防、上电、断电、寻车、开坐垫。
- 官方车辆和本地 BLE 车辆支持手动关联。
