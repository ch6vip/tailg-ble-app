...
> **实验例外**：分支 `feature/ble-adaptation` 在试做 BLE 恢复，进度见 [ble_adaptation_progress.md](ble_adaptation_progress.md)；未合入前不改变 master 产品边界。
...
8. [BLE 适配实验进度](ble_adaptation_progress.md)：**仅 `feature/ble-adaptation` 分支**；记录恢复路径与优先级，不代表 master 已改产品边界。
...
| BLE 实验 | [ble_adaptation_progress](ble_adaptation_progress.md) | **实验分支** | `feature/ble-adaptation` 恢复进度与优先级；未改 master 产品边界 |
